...
router.post('/message', (req, res) => {
  try {
    ...
    let response = {
      message: "I'm your AI medical assistant. How can I help with dosing today?",
      category: "general"
    };
    ...
    if (lowerMessage.includes('paracetamol') || lowerMessage.includes('acetaminophen')) {
      if (lowerMessage.includes('20kg') || lowerMessage.includes('20 kg')) {
        response.message = "For a child weighing 20kg, the standard paracetamol dose is 15mg/kg = 300mg per dose, every 6 hours. Maximum 4 doses per day (1200mg/24h).";
      } else {
        response.message = "Paracetamol dosing: 15mg/kg per dose, every 6 hours. Maximum 4 doses per day. Reduce dose in hepatic impairment.";
      }
    } 
    else if (lowerMessage.includes('ibuprofen')) {
      response.message = "Ibuprofen dosing: 10mg/kg per dose, every 8 hours. Maximum daily dose: 2400mg for adults, 30mg/kg/day for children. Take with food.";
    }
    else if (lowerMessage.includes('amoxicillin')) {
      response.message = "Amoxicillin dosing: 25-30mg/kg per dose, every 8-12 hours depending on infection severity.";
    }
      ...
      response.message = "To check drug interactions, please specify which medicines you'd like me to check.";
    }
    else {
      response.message = "I can help with dosing calculations, drug interactions, and safety information. Try asking: 'What's the dose of paracetamol for a 20kg child?'";
    }
